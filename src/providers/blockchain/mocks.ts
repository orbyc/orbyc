...
export function mockDataSource(
  erc245: ERC245Collection,
  erc423: ERC423Collection,
  timeout: number = 1000
): DataSource {
  return {
    erc245: {
      ...
      getAsset: (id) => {
        return new Promise((resolve) =>
          setTimeout(() => resolve(erc245.assets[id]), timeout)
        );
      },
      getAssetCertificates: (id) => {
        return new Promise((resolve) =>
          setTimeout(() => resolve(erc245.assetCertificates[id]), timeout)
        );
      },
      getAssetComposition: (id) => {
        return new Promise((resolve) =>
          setTimeout(
            () => resolve([erc245.parents[id], erc245.compositions[id]]),
            timeout
          )
        );
      },
      getAssetTraceability: (id) => {
        return new Promise((resolve) =>
          setTimeout(() => resolve(erc245.traceabilities[id]), timeout)
        );
      },
      getCertificate: (id) => {
        return new Promise((resolve) =>
          setTimeout(() => resolve(erc245.certificates[id]), timeout)
        );
      },
      getMovement: (id) => {
        return new Promise((resolve) =>
          setTimeout(() => resolve(erc245.movements[id]), timeout)
        );
      },
      getMovementCertificates: (id) => {
        return new Promise((resolve) =>
          setTimeout(() => resolve(erc245.movementCertificates[id]), timeout)
        );
      },
      issueAsset: () => Promise.resolve(true),
      issueCertificate: () => Promise.resolve(true),
      issueMovement: () => Promise.resolve(true),
      addAssetCertificates: () => Promise.resolve(true),
      addMovementCertificates: () => Promise.resolve(true),
      addMovements: () => Promise.resolve(true),
      addParents: () => Promise.resolve(true),
    },
    erc423: {
      ...
      hasRole: (address, role) =>
        Promise.resolve(erc423.roles[role].includes(address)),
    },
    utils: {
      currentAccount: () =>
        Promise.resolve("0x024269e2057b904d1fa6a7b52056a8580a85180f"),
    },
  };
}
...
export function getMockAsset() {
  ...
  assetMetadata.setBackground(
    getMockImage(
      "TestAssetMetadataBackground Name",
      "https://picsum.photos/500/200/?blur"
    )
  );
...
export function getMockTimestamp(date: string): Timestamp {
  return new Timestamp().setSeconds(Date.parse(date));
}
...
export const mockERC245: ERC245Collection = {
  ...
  certificates: {
    1: getMockCertificate(1),
    2: getMockCertificate(2),
    3: getMockCertificate(3),
  },
...
export const mockERC423: ERC423Collection = {
  ...
  agents: {
    ...
    "0x7B997BD00599a87334a4187e51A2320D740d14bb":
      "0xe375639d0Fa6feC13e6F00A09A3D3BAcf18A354F",
  },
  roles: {
